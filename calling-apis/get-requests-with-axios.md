...
Jamel is working on the JavaScript front-end of his app called DogSpotter, where he maps dogs he spots to the neighborhoods he lives in.

Jamel needs to get a source of dog breeds. He's not an expert on dogs, but he knows that there's an API that will help him get this data: [The Dog CEO API](https://dog.ceo/dog-api/documentation/).
...
- He will choose to use a Node package named `axios` to make API calls. He needs to learn the syntax for using this library.
...
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `const axios =`                                     | We will use the `const` variable `axios` and to hold our axios library functionality.                                                                                                                                              |
| `require('axios');`                                 | The `require` function finds the exported objects defined in another modules or file, and imports it. In this case, we are importing the exported objects defined in the `axios` module.                                           |
...
| `(response) => { ... }`                             | We pass in a function as the argument to `then()`. Here, we are passing in an anonymous arrow function. This function can anticipate one parameter: a parameter that represents to the HTTP response. We've named this `response`. |
| `.catch( ... )`                                     | This function called, which is _chained_ off of `axios.get().then()`, determines what happens if the request is _unsuccessful_ and throws an error. This function also expects one argument: a function.                           |
| `(error) => { ... }`                                | The function we pass into `catch` is an anonymous arrow function. This function can anticipate an argument that represents the error object.                                                                                       |
...
First, before Jamel implements his full API call, he wants to prove that _any_ valid API call sends in the first place! If the response is successful, he'll print to the console, "success!" If it's unsuccessful, he'll print "error!"
...
Assuming that the dependencies are working, we should get the following output from this code:
...
Note the `.` that is before `then` and `catch`. Let's pause on this syntax, because reading it now will help us read JavaScript more smoothly in the future.
...
1. `get()` is a function that is called onto `axios`
1. `then()` is a function that is called onto `axios.get()`
1. `catch()` is a function that is called onto `axios.get().then()`

These function calls are chained onto each other. `axios` is specifically looking for this syntax whenever we make an HTTP request.
...
Let's also inspect the anonymous functions we wrote. The function that we pass into `then()` is responsible for determining what happens when we get a successful response. This function takes in one argument, `response`.

Similarly, the function we pass into `catch()` is responsible for handling unsuccessful responses.
...
After looking at [the docs for Search/Forward Geocoding](https://locationiq.com/docs), Jamel determined that his request should look like this:
...
[
  {
    "place_id": "332439918643",
    "licence": "https://locationiq.com/attribution",
    "lat": "47.673455",
    "lon": "-122.331537",
    "display_name": "Seattle, King County, Washington, USA",
    "boundingbox": ["47.495514", "47.734165", "-122.435956", "-122.236044"],
    "importance": 0.25
  }
]
```

To add query params to our `GET` request, Jamel can research the `axios` documentation.
...
When he runs it, he indeed gets back the successful response he hoped for!

## Observing Asynchronicity

This API call is made _asynchronously_. This asynchronous nature will affect Jamel approaches his project. But first, how can he observe the asynchronous property of this code?

Let's run this code and observe:
...
Reading through this code from top-to-bottom, without understanding asynchronosity, would lead us to believe that:
...
However, because the API call happens asynchronously, our program doesn't wait for the API response to come back before proceeding with the program. The result is that our `dogImageURL` comes back as `undefined`.
...
JavaScript features a lot of ways to work with asynchronous code. Different search terms to use to research more could be "JavaScript Promises" and "JavaScript async await."
