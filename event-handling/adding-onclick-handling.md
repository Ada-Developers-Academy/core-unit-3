...
In order to add our behavior, let's start by defining our _event handler_ in [our JavaScript code](https://replit.com/@adacore/Adding-onclick-Handling-Demo#scripts/index.js). An **event handler** is a function whose responsibility is to perform necessary, required, or desired actions after a certain event is fired.
...
Let's finish wiring up [our Crab Site example](https://replit.com/@adacore/Adding-onclick-Handling-Demo#scripts/index.js)! We want the `addCrab` handler to run every time we click an "Add Crab" button.
...
[Let's change our JS code](https://replit.com/@adacore/Adding-onclick-Handling-Demo#scripts/index.js) to access the event object that comes into every event handler. Notice that we are only changing two things:
