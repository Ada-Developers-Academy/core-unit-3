...
She might start her code like this, creating a `cookieButton` object, which has a `cookieCount` property. But how can she increase the `cookieCount`? More importantly, how could she set it up so that the `cookieButton` object can increase its `cookieCount` when it's clicked?
...
She could write a function that accesses the `cookieButton`'s property...
...
But this feels fishy to her: the `onCookieButtonClick` function modifies the `cookieButton` object, even though the two are unrelated.

Ideally, Ember would like the `cookieButton` object to manage its own behavior, which modifies its own properties.
...
We would also like to attach methods to objects in the same way!
...
We have seen this form before, which continues to be valid. We can create a function outside the object, and bring it into the object using an expression that evaluates to the function reference.
...
| <div style="min-width:200px;"> Piece of Code </div> | Notes                                                                                   |
| --------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `const longMeow = function() { ... }`               | We create the function `longMeow` outside the object, storing a reference to the function in the `longMeow` variable.                                   |
| `meow: longMeow`                                    | This establishes a property named `meow`, whose value is the function reference stored in the `longMeow` variable. |
...
When we add methods to our object with non-arrow functions, we can access the other members using the keyword `this`.

`this` is a keyword we can access inside any object definition. All objects and all functions have a `this` value!

Inside a method, the value of `this` changes based on how the method is called.

_When inside a method that is inside an object_, _when we call the method through the object (using dot syntax)_, `this` refers to the object.
...
Non-arrow functions and arrow functions define `this` differently.

| Non-arrow functions                                                  | <div style="min-width:330px;">Arrow functions</div>                             |
| -------------------------------------------------------------------- | ------------------------------------------- |
